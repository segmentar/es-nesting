...
namespace Core.Shared
{
    ...
    {
        public static SNNested MakeNestedForwardCollider(SGSymbol symbol, SGDelimiter delimiter, SGConditional conditional, SGText text, Boolean debug)
        {
            SNNested nestedResult = default;

            Debug_ForwardCollider(symbol, delimiter, conditional, text, debug);
            ...
            do
            {
                var token = STToken.Runtime_MakeTokenNextIndex(symbol, 0, move, position, SAConstantDisagreement.QueryForwardDefault, text_REPLACED);
