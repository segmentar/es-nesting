...
namespace Core.Shared
{
    ...
    {
        public static void Debug_ForwardCollider(SGSymbol symbol, SGDelimiter delimiter, SGConditional conditional, SGText text, Boolean debug)
        {
            goto descriptor;
            ...
            descriptor:
            {
                if (debug is true)
                {
                    var message = String.Join('\n'.ToString(), new String[] {
                        String.Empty + nameof(SNNested) + ' ' + "::" + ' ' + nameof(Debug_ForwardCollider) + ' ' + '{',
